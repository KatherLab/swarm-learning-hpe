...
for model, model_results in data.items():
    for category, category_results in model_results.items():
        for result in category_results:
            all_data.append([model, category, result])

df = pd.DataFrame(all_data, columns=["Model", "Category", "Result"])

# Create a violin plot
plt.figure(figsize=(20,10))

sns.violinplot(x="Model", y="Result", hue="Category", data=df, palette="Set2", split=False)
