...
sudo $script_dir/../../swarm_learning_scripts/run-sn -it --rm --name=sn"$ip_addr" --network=host-"$ip_addr"-net --host-ip="$ip_addr" --sentinel --sn-p2p-port=30303 --sn-api-port=30304 --key=workspace/"$workspace"/cert/sn-"$ip_addr"-key.pem --cert=workspace/"$workspace"/cert/sn-"$ip_addr"-cert.pem --capath=workspace/"$workspace"/cert/ca/capath --apls-ip="$sentinal" --apls-port 5000
