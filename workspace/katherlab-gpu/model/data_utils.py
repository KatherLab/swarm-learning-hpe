...
def SortClini_SlideTables(imagesPath, cliniTablePath, slideTablePath, label, outputPath, reportFile, csvName):
    ...
    for item in range(len(imagesPath)):
        ...
        reportFile.write(
            '{} Patients didnt have the proper label for target label: {}'.format(lenBefore - lenafter, label) + '\n')
        print('**********************************************************************')
        ...
        reportFile.write('Data for {} Patients from Clini Table is not found in Slide Table!'.format(
        ...
        reportFile.write('Data for {} Patients from Slide Table is not found in Clini Table!'.format(
...
def ConcatCohorts_Classic(imagesPath, cliniTablePath, slideTablePath, label, reportFile):
    ...
    for imgCounter in range(len(imagesPath)):
        ...
        notAcceptedValues = ['NA', 'NA ', 'NAN', 'N/A', 'na', 'n.a', 'N.A', 'UNKNOWN', 'x', 'NotAPPLICABLE',
                             'NOTPERFORMED',
                             'NotPerformed', 'Notassigned', 'excluded', 'exclide', '#NULL', 'PerformedButNotAvailable',
                             'x_', 'NotReported', 'notreported', 'INCONCLUSIVE']
