...
from data_utils import Get_split_loader
...
def Train_MIL_CLAM(datasets, cur, args, trainFull=False):
    ...
    for epoch in range(args.max_epochs):
        ...
        if args.model_name in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:
            ...
            Train_loop_MIL(epoch=epoch, model=model, loader=train_loader, optimizer=optimizer,
                           n_classes=args.num_classes, writer=writer, loss_fn=loss_fn)
            if not trainFull:
...
def Validate_MIL(cur, epoch, model, loader, n_classes, early_stopping=None, writer=None, loss_fn=None,
                 results_dir=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    acc_logger = Accuracy_Logger(n_classes=n_classes)
...
def Summary(model, loader, n_classes):
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        patient_results.update(
            {slide_id: {'case_id': case_id, 'slide_id': slide_id, 'prob': probs, 'label': label.item()}})
        error = utils.calculate_error(Y_hat, label)
        test_error += error
...
def Train_model_Classic(model, trainLoaders, valLoaders=[], criterion=None, optimizer=None, num_epochs=25,
                        is_inception=False, results_dir=''):
    since = time.time()
