...
for skip, temporal, middleframe in tqdm(combinations):
    ...
    trial_name = f"Appendectomy_Classification_swarm_temp{temporal}_mid{middleframe}_{frames}frames"
    ...
    with open(data_folder+"/centers.json", "r") as file:
        ops = json.load(file)

    for c in ops:
        
        for f in ops[c]:
            op_path = os.path.join(data_folder, c, f)
            for id, val in ops[c][f]:
                logprint(f"{op_path}, {id}, {val}")
                if val == 0:
                    if debug and len(train_sets) > 3:
                        continue
                    
                    set = Dataloader.AppendectomyDataset(op_path, str(id), width, height, transform_train, middleframe, skip)
                    weights[set.lbl] += 1
                    if temporal:
                        train_sets.append(torch.utils.data.DataLoader(set, batch_size=batch_size, shuffle=False, num_workers=num_workers))
                    else:
                        train_sets.append(set)
                else:
                    if debug and len(val_sets) > 3:
                        continue
                    set = Dataloader.AppendectomyDataset(op_path, str(id), width, height, transform_val, not temporal, skip)
                    if temporal:
                        val_sets.append(torch.utils.data.DataLoader(set, batch_size=batch_size, shuffle=False, num_workers=num_workers))
                    else:
                        val_sets.append(set)
