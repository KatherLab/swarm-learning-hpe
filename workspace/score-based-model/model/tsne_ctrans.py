...
# Function to perform t-SNE and plot results with convex hulls for specific labels
def perform_tsne_and_plot(directories, labels):
    all_features = []
    all_labels = []
    ...
    for label in np.unique(all_labels):
        ...
        if label in ['KIRP Original', 'KIRC Original']:
            hull = ConvexHull(tsne_result[indices])
            for simplex in hull.simplices:
                plt.plot(tsne_result[indices][simplex, 0], tsne_result[indices][simplex, 1], 'k-')
            plt.fill(tsne_result[indices][hull.vertices, 0], tsne_result[indices][hull.vertices, 1], alpha=0.3,
                     label=label, color=color_dict[label])
        else:
            plt.scatter(tsne_result[indices, 0], tsne_result[indices, 1], label=label, color=color_dict[label])

    plt.xlabel('t-SNE Component 1')
    plt.ylabel('t-SNE Component 2')
    plt.title('t-SNE of Pathology Features')
    plt.legend()
    plt.show()
...
perform_tsne_and_plot(directories, labels)
