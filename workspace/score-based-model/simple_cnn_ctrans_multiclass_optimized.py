...
# Get class weights and apply them to CrossEntropyLoss
class_weights = get_class_weights(train_datasets)
weights = torch.tensor([class_weights[i] for i in range(4)], dtype=torch.float)
...
# Optional: Create a weighted sampler for oversampling the minority class
def make_weighted_sampler(datasets):
    all_labels = []
    for dataset in datasets:
        for _, label in dataset:
            all_labels.append(label.item())
...
sampler = make_weighted_sampler(train_datasets)
