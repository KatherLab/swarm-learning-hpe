...
class AutoencoderKL(nn.Module):
    ...
    def forward(self, x: torch.Tensor, sampled = False, *args) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        z_mu, z_sigma = self.encode(x)
        z = self.sampling(z_mu, z_sigma)
        if args:#if latent is given as inputs
            reconstruction = self.decode(args[0])
        else:
